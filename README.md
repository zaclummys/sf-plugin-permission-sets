...
`npm run test:unit` covers `src/core/`, `src/services/`, and `src/ui/`, at 100% statements, branches, functions, and lines: `test/unit/**/*.test.ts`, mocha, and chai. Those are the three layers a test can reach without spawning the CLI, which is what lets the gate run on every push, in under a second. Core and ui are pure and stay real. A service's only boundaries are the org, the confirmation prompt, and the filesystem, so the first two are hand-written fakes of the ports the service already takes, and the third is a real temp directory. Nothing is module-mocked. The gate is a c8 config of its own ([`.c8rc.gate.json`](.c8rc.gate.json)), so a branch no test reaches fails the run rather than showing up as a number nobody reads.

See [UNIT_TESTING.md](UNIT_TESTING.md) for why the suite reads `lib/` rather than `src/`, what the gate changed in `src/` on its way to 100%, and what the Salesforce CLI ecosystem does differently.
