...
- **Adapters** (`src/adapters/`): the boundary to the outside world, one directory per adapter. `connection-org-client/` implements the `OrgClient` port with a Salesforce `Connection`, and owns all the SOQL and SObject detail: the client in `client`, the query text in `soql`, the Collections API grouping and chunking in `dml`. Those two are the client's own modules rather than adapters of their own, which is why they live inside its directory instead of beside it. Services depend on the port, not the SDK.
