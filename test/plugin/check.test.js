...
describe('sf ps check', () => {
    it('passes a valid file with exit 0', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', validPath]);
    ...
    it('reports warnings but exits 0', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', warningsPath]);
    ...
    it('turns warnings into a failure under --strict', async ({ expect }) => {
        const { stdout, stderr, exitCode } = await runPs(['ps', 'check', '--file', warningsPath, '--strict']);
    ...
    it('leaves a clean file passing under --strict', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', validPath, '--strict']);
    ...
    it('fails a schema violation with exit 1', async ({ expect }) => {
        const { stdout, stderr, exitCode } = await runPs(['ps', 'check', '--file', schemaErrorPath]);
    ...
    it('fails malformed YAML with exit 1', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', malformedPath]);
    ...
    it('errors when no file matches the glob', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', 'test/fixtures/nope/*.yml']);
    ...
    it('aggregates findings across multiple -f files', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', validPath, '--file', warningsPath]);
    ...
    it('emits a valid --json envelope on success', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', warningsPath, '--json']);
    ...
    it('exits 1 but still emits valid --json on failure', async ({ expect }) => {
        const { stdout, exitCode } = await runPs(['ps', 'check', '--file', schemaErrorPath, '--json']);
