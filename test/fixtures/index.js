// Paths to the YAML fixtures a vitest spec passes to `-f`, plus the org values the diff
// fixtures lean on. These are file paths, not file contents: a spec hands them straight to
// `sf ps`, which reads them itself. They stay relative to the project root because runPs
// spawns `sf` with cwd there.
//
// Only what a vitest spec still imports lives here. The NUTs reach the same directory
