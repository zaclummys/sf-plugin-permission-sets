...
>
> The split is by whether a test needs an org, because that decides where it can run. `test/nut/*.nut.ts` needs none and runs on every pull request including a fork's (`npm test`). `test/nut/org/*.nut.ts` needs a Dev Hub, creates its own scratch org from `test/nut/project`, and runs separately (`npm run test:org`).
>
...
- A spec that needs org state builds it, through `test/helpers/org-state.js`: `createPermissionSet` makes one named after that spec and deletes it (with its assignments) when the spec finishes, and `grant` assigns it. A permission set and an assignment are plain sObjects, so this costs milliseconds and nobody has to be told the org already holds something. Pass the spec's own `onTestFinished` (`async ({ expect, onTestFinished })`), never the import from `vitest`: under `sequence.concurrent` the imported hook cannot tell which spec is calling and files the cleanup under whichever one is running.
- The org is split in two so those specs cannot collide with the ones that read it. Specs build state on `islandUser` and every read of the org is scoped to `--user declaredUser`. A permission set that appears and disappears mid-run is therefore never inside a snapshot or on either side of a before/after comparison, which is what a second `ps export` in one spec really is.
- Users and permission set licenses are the two things a spec cannot build: a user cannot be deleted, only deactivated, and a license comes with the org's edition. Those stay named in `test/fixtures/org.js`, and that file is the whole list of what pointing the suite at another org means updating.
