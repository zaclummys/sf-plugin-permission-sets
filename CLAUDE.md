...
- `src/core/` stays pure: no `@salesforce/*` imports, no I/O, no CLI concerns. Plain data, functions, and the domain classes below. Enforced: `core/` may not import `node:*` or `globby`, so a file is handed its text and `services/` owns the disk in both directions (`CheckService` reads, `ExportService` writes).
...
> One runner, two suites, split by what they drive. **Mocha** throughout, and neither suite mocks a module, fakes a timer, or imports `src/`: both read the working tree's `lib/`, never the published package. The scripts mirror the three globs one for one: `test:unit`, `test:nut`, and `test:nut:org`, the last nested because an org spec is a NUT that happens to need a Dev Hub, which is also how the directories are laid out. A prefix runs everything under it that needs no credentials, so `npm test` is `test:unit` plus `test:nut`, which is what a fork's pull request can run, and the org half stays opt-in because it costs a scratch org.
>
...
>
> **Unit tests**, `test/unit/**/*.test.ts`, with chai, gated at 100% over `core/`, `services/`, and `ui/` (`npm run test:unit`). Those are the three layers reachable without spawning the CLI, which is why the gate runs on every push. `core/` and `ui/` are pure and stay real. A service's boundaries are the `OrgClient` port, the `ConfirmDeletions` callback, and the filesystem, so the first two are hand-written fakes of ports the service already takes and the third is a real temp directory. The rules that differ there (erasable syntax only, job files as files, why the gate owns its own dump directory, and the rule for a defensive branch the gate cannot reach) live in [UNIT_TESTING.md](UNIT_TESTING.md). Everything under "Testing principles" below applies to both.
>
> The layout is one directory per command, and inside it the split is by whether a test needs an org, because that decides where it can run. `test/nut/<command>/*.nut.ts` needs none and runs on every pull request including a fork's (`npm run test:nut`). `test/nut/<command>/org/*.nut.ts` needs a Dev Hub, uses a scratch org built from `test/nut/project`, and runs separately (`npm run test:nut:org`). The two globs never overlap, which is what keeps a spec needing an org out of the fork-safe run. `run.ts` and `org-session.ts` sit above the command directories because every command shares them.
>
...
- The environment the suite depends on is pinned in the wireit `env` block for `test:nut` and `test:nut:org` (`NO_COLOR`, `FORCE_COLOR`, `SF_DISABLE_TELEMETRY`), never read from the machine the suite happens to run on.
